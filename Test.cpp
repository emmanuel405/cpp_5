...
TEST_CASE("accumulate"){
	...
	for (int i: accumulate(range(5,15))){
        tmp += i;
...
TEST_CASE("filterfalse"){
    ...
    for (auto i: filterfalse(Under10{}, vecInt))
...
TEST_CASE("compress"){
    ...
    for (auto i: compress(string("abcdefghij"), vector<bool>({true,true,false,true,false,false,true,true,true,false})) ){
        c = s.at(j++);
