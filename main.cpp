#include <iostream>
#include <vector>

#include "range.hpp"
#include "accumulate.hpp"



using namespace itertools;
using namespace std;

int main() {

    // cout << "####  range:  ####";
	// for (int i: range(5,9))
	// 	cout << i << " "<<endl;      // 5 6 7 8

    vector<int> vecInt;
    vecInt.push_back(1);vecInt.push_back(2);vecInt.push_back(3);vecInt.push_back(4);
    vector<string> vecString;
    vecString.push_back("Hello");vecString.push_back("Bye");vecString.push_back("Adam");
    vector<float> vecFloat;
    vecFloat.push_back(-1);vecFloat.push_back(0.3);vecFloat.push_back(5.2);vecFloat.push_back(-8.3);

    cout << "####  accumulate:  ####";
    cout << endl << "accumulate of range: " << endl;
	for (int i: accumulate<vector<int> >(vecInt))
		cout << i << " ";      // 5 11 18 26

    // cout << endl << "accumulate of vector<string>: " << endl;
    // for (auto i: accumulate(vecString) )
    //     cout << i << " ";  // Hello HelloBye HelloByeAdam
    // cout << endl << endl;

    // cout << endl << "accumulate of range with binary operator: " << endl;
	// for (int i: accumulate(range(5,9), [](int x, int y){return x*y;}))
	// 	cout << i << " ";      // 5 30 210 1680

    return 0;
}